...
When issuing for another machine, the target should send a profile generated by `scripts/profile_device.py`. The profile includes:
...
The profile does not contain the Keychain secret. It gives the issuer enough material to encrypt the payload for that machine and to bind the policy to the target fingerprint.
